...
let extractConnection (wModel : Model) (cId : ConnectionId) : Connection =
    ...
    let ConnectionId strId, InputPortId strInputPort, OutputPortId strOutputPort = conn.Id, conn.InputPort, conn.OutputPort
    {
        ...
        Vertices = ( [conn.Segments[0].Start.X,conn.Segments[0].Start.Y] @ List.mapi (fun i seg -> (seg.End.X,seg.End.Y)) conn.Segments)
    ...
    let renderSegment (segment : Segment) (colour : string) (width : string) (index:int) (numofSegments:int) : ReactElement =
        ...
        if SegDirection = Horizontal then
            ...
            let renderWireSubSegment (vertex1 : XYPos) (vertex2 : XYPos) (renderJumpSeg:RenderJumpSegmentType): list<ReactElement> =
                ...
                if ((rendertype=Penultimate)&&(nextSegment.Dir=Horizontal)&&(segment.Dir=Horizontal)&&(abs(Xb-Xa) < 20)&&(abs(nextSegment.End.X-nextSegment.Start.X) < 20)) then 
                                                                                                                [makeLine  Xb Ya Xb Yb lineParameters]

                
                elif ((rendertype=Last)&&(prevSegment.Dir=Horizontal)&&(abs(Xb-Xa) < 20)) then 
                                                        [makeLine  Xb Ya Xb Yb lineParameters]

                elif ((abs(nextSegment.End.Y-nextSegment.Start.Y) < 20.0)&&(rendertype<>Last)&&(rendertype<>First)&&(abs(prevSegment.End.Y-prevSegment.Start.Y) > radiusSize)) then 
                                                        [makeLine (Xa+radiusSize) Ya Xb Yb lineParameters]

                elif ((abs(nextSegment.End.Y-nextSegment.Start.Y) < 20.0)&&(rendertype<>Last)&&(rendertype<>First)&&(abs(prevSegment.End.Y-prevSegment.Start.Y) < radiusSize)) then 
                                                        [makeLine  Xa Ya Xb Yb lineParameters]

                elif ((rendertype=Second)&&(prevSegment.Dir=Horizontal)&&(abs(Xb-Xa) < 8)) then 
                                                                                                                let isDowntoUp = if( nextSegment.End.Y - nextSegment.Start.Y < 0) then true
                                                                                                                                 else false
                ...
                elif ((rendertype=First)&&(nextSegment.Dir=Horizontal)&&(abs(nextSegment.End.X-nextSegment.Start.X) < 8)) then 
                                                        [makeLine  Xa Ya (Xa+5.0) Yb lineParameters]

                

                
                elif ((abs(prevSegment.End.Y-prevSegment.Start.Y) < 20.0)&&(rendertype<>Last)&&(rendertype<>First)) then 
                                                                                                                let isDowntoUp = if( nextSegment.End.Y - nextSegment.Start.Y < 0) then true
                                                                                                                                 else false
                                                                                                                if (isDowntoUp = true) then 
                                                                                                                                    let startingPoint = {X = Xb - radiusSize ; Y = Yb}
                ...
                elif ((rendertype = First)&&(abs(nextSegment.End.Y-nextSegment.Start.Y) < radiusSize)) then
                                                                                                            [makeLine Xa Ya Xb Yb lineParameters]
                ...
                elif ((rendertype = Third)&&(abs(prevSegment.End.Y-prevSegment.Start.Y) > radiusSize)) then                                                            
                                                                                                            let isDowntoUp = if( nextSegment.End.Y - nextSegment.Start.Y < 0) then true
                                                                                                                                else false
                                                                                                            if (isDowntoUp = true) then 
                                                                                                                                    let startingPoint = {X = Xb - radiusSize ; Y = Yb}
                ...
                elif ((rendertype = Antepenultimate)&&(abs(nextSegment.End.Y-nextSegment.Start.Y) > radiusSize)) then                                                            
                                                                                                            let isDowntoUp = if( nextSegment.End.Y - nextSegment.Start.Y < 0) then true
                                                                                                                                else false
                                                                                                            if (isDowntoUp = true) then 
                                                                                                                                    let startingPoint = {X = Xb - radiusSize ; Y = Yb}
                ...
                elif ((rendertype = Last)&&(abs(prevSegment.End.Y-prevSegment.Start.Y) > radiusSize)) then                                                            
                                                                                                            //[makeLine (Xa+radiusSize) Ya Xb Yb lineParameters;]
                                                                                                            [makeLine (Xa+radiusSize) Ya Xb Yb lineParameters;]
                ...
                elif ((nextSegment.Dir = Vertical) && (rendertype = Middle) && (isseg1LeftToRight = true)) then
                                                                                                                let isDowntoUp = if( nextSegment.End.Y - nextSegment.Start.Y < 0) then true
                                                                                                                                 else false
                                                                                                                if (isDowntoUp = true) then 
                                                                                                                                    let startingPoint = {X = Xb - radiusSize ; Y = Yb}
                ...
                elif((nextSegment.Dir = Vertical) && (rendertype = Middle) && (isseg1LeftToRight = false)) then 
                                                                                                            let isDowntoUp = if( nextSegment.End.Y - nextSegment.Start.Y < 0) then true
                                                                                                                                else false
                                                                                                            if (isDowntoUp = true) then 
                                                                                                                                    let startingPoint = {X = Xb + radiusSize ; Y = Yb}
            ...
            let completeWireSegmentRenderFunction (seg : Segment) : list<ReactElement> =
                ...
                match jumpCoordinateList with
                    | [] -> renderWireSubSegment seg.Start seg.End NoJump
                    ...
                    | lst ->
                        ...
                        if (segment.Start.X > segment.End.X) then
                            ...
                            @
                            renderWireSubSegment {X = lastSegmentJumpCoordinate - segmentJumpHorizontalSize/2.0; Y = y} seg.End After
            ...
            let segmentElements = 
                ...
                elif (nextSegment.Dir = Vertical) then 
                                                        //printfn "%A" (snd(segmentTuple))
                                                        [makeLine Xa Ya Xb Yb lineParameters]
                ...
                elif ((nextSegment.Dir = Horizontal) && (isseg1DowntoUp = true)) then
                                                                                                        let isLeftToRight = if( (nextSegment.End.X - nextSegment.Start.X) >= 0) then true
                                                                                                                            else false
                ...
                elif((nextSegment.Dir = Horizontal) && (isseg1DowntoUp = false)) then 
                                                                                                        let isLeftToRight = if( nextSegment.End.X - nextSegment.Start.X >= 0) then true
                                                                                                                            else false
                                                                                                        if (isLeftToRight = true) then 
                                                                                                                                let startingPoint = {X = Xb ; Y = Yb - radiusSize}
            ...
            let completeWireSegmentRenderFunction (seg : Segment) : list<ReactElement> =
                ...
                let jumpCoordinateList =
                    match displayMode with 
                                                | OldFashionedCircuit ->  if (segment.Start.X > segment.End.X) then
